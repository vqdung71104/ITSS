...
import { getGroups as getGR } from "../data/groupData";
...
export const getGroups = async (projectId?: string) => {
  ...
  const mockGroups = response.data;
...
export const addGroupMember = async (
  ...
  // }).then(handleResponse);
  await axiosInstance.post(`/groups/${groupId}/add-member/${member.id}`, {
    group_id: groupId,
    memberId: member.id,
  });
  // window.location.href = "/dashboard/groups";
...
export const changeGroupLeader = async (
  ...
  // }).then(handleResponse);
  await axiosInstance.put(`/groups/${groupId}/change-leader/${newLeaderId}`, {
    group_id: groupId,
    newLeaderId,
  });
  const groupIndex = mockGroups.findIndex((group) => group.id === groupId);
  if (groupIndex !== -1) {
    ...
    mockGroups[groupIndex].members = mockGroups[groupIndex].members.map(
      (member) => ({
        ...member,
...
const loadMockGroups = async () => {
  mockGroups = await getGR();
  console.log("mockGroups1", mockGroups); // In ra sau khi mockGroups được cập nhật
};
loadMockGroups();
console.log("mockGroups2", mockGroups); // In ra ngay sau khi gọi loadMockGroups
